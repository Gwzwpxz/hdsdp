...
elseif method == "direct"
    ...
    nthresh = ceil(0.2 * length(coneidx));
    
    % Analysis
    [vmin, vminid] = mink(abs(v(coneidx)), nthresh);
    xvmin = x(m + vminid);
    [xmin, xminid] = mink(x(coneidx), nthresh);
%     xminid = xminid(xmin < mean(xmin));
    interxvmin = intersect(vminid, xminid);   
    v(xminid) = 0;
