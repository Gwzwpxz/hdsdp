...
extern void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) {
    ...
    if ( nrhs == 4 ) {
        ...
        param = mxGetField(params, 0, "maxPCIter");
        if ( param ) { maxPCIter = (int) (*mxGetPr(param)); }
        ...
        param = mxGetField(params, 0, "barStartTol");
        if ( param ) { barStartTol = (double) (*mxGetPr(param)); }
    ...
    potlp->dblParams[DBL_PARAM_BARSTARTTOL] = barStartTol;
