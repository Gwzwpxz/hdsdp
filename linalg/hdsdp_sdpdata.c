...
static int dataMatRankOneSparseIsEye( void *A, double *dEyeMultiple ) {
    
    return 0;
}

static int dataMatRankOneDenseIsEye( void *A, double *dEyeMultiple ) {
...
static int dataMatSparseIsUnitCol( void *A, int *iUnitCol ) {
...
static int dataMatRankOneDenseIsUnitCol( void *A, int *iUnitCol ) {
...
static void dataMatRankOneSparseKKT2SolveRankOneImpl( void *A, hdsdp_linsys *S, double *Sinv, double *sign, double *v ) {
    ...
    if ( spr1->nSpR1FactorElem >= 0.3 * spr1->nSDPCol ) {
        ...
        for ( int iElem = 0; iElem < spr1->nSpR1FactorElem; ++iElem ) {
            double ai = spr1->spR1MatElem[iElem];
            axpy(&spr1->nSDPCol, &ai, Sinv + spr1->spR1MatIdx[iElem] * spr1->nSDPCol,
...
/*==========================================================================================*/
static double dataMatZeroKKT3DotDenseImpl( void *A, double *B, double *aux ) {
    
    return 0.0;
}

static double dataMatSparseKKT3DotDenseImpl( void *A, double *B, double *aux ) {
...
static double dataMatDenseKKT3DotDenseImpl( void *A, double *B, double *aux ) {
...
static double dataMatRankOneSparseKKT3DotDenseImpl( void *A, double *B, double *aux ) {
...
static double dataMatRankOneDenseKKT3DotDenseImpl( void *A, double *B, double *aux ) {
...
/* Now that we have 4 types of non-trivial data matrices (excluding zero matrices),
   ...
   However, we ASSERT that dense matrices will NEVER invoke this method
...
static double KKT5Pair_Sparse_Dense( sdp_coeff_sparse *A, sdp_coeff_dense *B, double *Sinv, double *aux ) {
    /* Implement trace(A * S^-1 * B * S^-1 ) for sparse A and dense B */
    double dTraceSinvASinvB = 0.0;
    int iAElem = 0;
    int iARow = 0;
    int iACol = 0;
...
static double KKT5Pair_Sparse_RankOneSparse( sdp_coeff_sparse *A, sdp_coeff_spr1 *B, double *Sinv, double *aux ) {
    ...
    return 2.0 * B->spR1FactorSign * dTraceSinvASinvB;
}

static double KKT5Pair_Sparse_RankOneDense( sdp_coeff_sparse *A, sdp_coeff_dsr1 *B, double *Sinv, double *aux ) {
    /* Implement trace(A * S^-1 * B * S^-1 ) for sparse A and rank-one dense B
        Note that B = sign * a * a' and
    ...
    /* Compute v */
    fds_symv(B->nSDPCol, 1.0, Sinv, B->r1MatFactor, 0.0, dSinvAVec);
    return B->r1FactorSign * tsp_quadform(A->nSDPCol, A->nTriMatElem, A->triMatRow, A->triMatCol, A->triMatElem, dSinvAVec);
}
...
static double KKT5Pair_RankOneSparse_RankOneDense( sdp_coeff_spr1 *A, sdp_coeff_dsr1 *B, double *Sinv, double *aux ) {
    /* Implement trace(A * S^-1 * B * S^-1 ) for sparse rank one A and dense rank one B
      trace(A * S^-1 * B * S^-1 )
    ...
    */
    ...
    double dSinvAVecDotBVec = dot(&A->nSDPCol, dSinvAVec, &HIntConstantOne, B->r1MatFactor, &HIntConstantOne);
...
static double dataMatSparseKKT5TraceASinvBSinvImpl( void *A, sdp_coeff *B, double *Sinv, double *aux ) {
    ...
    switch (B->dataType) {
        ...
        case SDP_COEFF_DENSE:
            return KKT5Pair_Sparse_Dense(sparse, B->dataMat, Sinv, aux);
            break;
        case SDP_COEFF_SPR1:
            return KKT5Pair_Sparse_RankOneSparse(sparse, B->dataMat, Sinv, aux);
            break;
        case SDP_COEFF_DSR1:
            return KKT5Pair_Sparse_RankOneDense(sparse, B->dataMat, Sinv, aux);
        default:
            assert( 0 );
            break;
    }
...
static double dataMatRankOneSparseKKT5TraceASinvBSinvImpl( void *A, sdp_coeff *B, double *Sinv, double *aux ) {
    ...
    switch (B->dataType) {
        ...
        case SDP_COEFF_SPARSE:
            ...
            break;
        case SDP_COEFF_DENSE:
            return KKT5Pair_RankOneSparse_Dense(spr1, B->dataMat, Sinv, aux);
            break;
        case SDP_COEFF_SPR1:
            return KKT5Pair_RankOneSparse_RankOneSparse(spr1, B->dataMat, Sinv, aux);
            break;
        case SDP_COEFF_DSR1:
            return KKT5Pair_RankOneSparse_RankOneDense(spr1, B->dataMat, Sinv, aux);
            break;
        default:
            assert( 0 );
            break;
...
static void sdpDataMatIChooseType( sdp_coeff *sdpCoeff, sdp_coeff_type dataType ) {
    ...
    switch ( dataType ) {
        case SDP_COEFF_ZERO:
            ...
            sdpCoeff->kkt3AdotB = dataMatZeroKKT3DotDenseImpl;
        ...
        case SDP_COEFF_SPARSE:
            ...
            sdpCoeff->iseye = dataMatSparseIsEye;
            sdpCoeff->isunitcol = dataMatSparseIsUnitCol;
            ...
            sdpCoeff->kkt3AdotB = dataMatSparseKKT3DotDenseImpl;
        ...
        case SDP_COEFF_DENSE:
            ...
            sdpCoeff->kkt3AdotB = dataMatDenseKKT3DotDenseImpl;
        ...
        case SDP_COEFF_SPR1:
            ...
            sdpCoeff->iseye = dataMatRankOneSparseIsEye;
            sdpCoeff->isunitcol = dataMatRankOneSparseIsUnitCol;
            ...
            sdpCoeff->kkt3AdotB = dataMatRankOneSparseKKT3DotDenseImpl;
        ...
        case SDP_COEFF_DSR1:
            ...
            sdpCoeff->iseye = dataMatRankOneDenseIsEye;
            sdpCoeff->isunitcol = dataMatRankOneDenseIsUnitCol;
            ...
            sdpCoeff->kkt3AdotB = dataMatRankOneDenseKKT3DotDenseImpl;
...
extern double sdpDataMatKKT5TraceASinvBSinv( sdp_coeff *sdpCoeff, sdp_coeff *sdpCoeff2, double *Sinv, double *dAuxiMat ) {
    ...
    return sdpCoeff->kkt5AsinvBsinv(sdpCoeff->dataMat, sdpCoeff2, Sinv, dAuxiMat);
}

extern double sdpDataMatKKT5SinvADotSinv( sdp_coeff *sdpCoeff, hdsdp_linsys *dualFactor, double *dInvMatrix,
                                         double *dAuxiMat) {
    
    return sdpCoeff->kkt5SinvAdotSinv(sdpCoeff->dataMat, dualFactor, dInvMatrix, dAuxiMat);
}
