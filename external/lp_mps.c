...
#define POTLP_INIT(var, type, size) (var) = (type *) calloc((unsigned long) (size), sizeof(type))
#define POTLP_MEMCPY(dst, src, type, size) memcpy(dst, src, (unsigned long) (sizeof(type) * (unsigned long) (size)))
#define POTLP_ZERO(var, type, size) memset(var, 0, (unsigned long) (sizeof(type) * (unsigned long) (size)))
#define POT_CALL(func) if ((func) != 0) {     \
                            retcode = 1;  \
                            goto exit_cleanup;         \
                        }
#endif
...
static int potDictCreate( pot_dict **pDict, int nMaxElem ) {
...
static int potDictAddElem( pot_dict *dict, char *key, int val ) {
...
static unsigned int potDictMapElem( pot_dict *dict, char *key ) {
...
static void potDictClear( pot_dict *dict ) {
...
static void potDictDestroy( pot_dict **pDict ) {
...
int potLpMpsRead( char *fname, char *name, int *pnRow, int *pnEqRow, int *pnInEqRow, int *pnCol, int *pnElem,
    ...
    /* Prepare matrix data */
    colMat = dcs_spalloc(nRow, nCol, nNz, 1, 1);
    ...
    for ( iCol = 0; !feof(mps); ) {
        ...
        if ( strcmp(rname, objname) == 0 ) {
            ...
            if ( dcs_entry(colMat, iRow, iCol, dElem) != 1 ) {
                retcode = 1;
                goto exit_cleanup;
            }
        }
